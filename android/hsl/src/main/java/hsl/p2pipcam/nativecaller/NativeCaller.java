...
public class NativeCaller {
    ...
    static {
        try {
            ...
            System.out.println("load lib error " + e.getMessage());
        }
    }
    ...
    public native static int SearchRecordFile(long nUserID, int bYear, int bMon, int bDay, int bHour, int bMin, int bSec,
                                              int eYear, int eMon, int eDay, int eHour, int eMin, int eSec, int file_mode, int file_index);
     ...
     * 以下为新增的方法
     */
    public native static int Connect(long UserID);

    public native static int StartBat(long userid, int enableLanSearch);

    public native static int StartStream3(long UserID, int StreamId, int subStreamId, int audiofmt);
}
