...
public class GLThread extends Thread {
    ...
    public void run() {
        ...
        if (LOG_THREADS) {
            Log.i("gltext", "GLThread starting tid=" + getId());
        }
    ...
    private void guardedRun() throws InterruptedException {
        ...
        try {
            ...
            while (true) {
                synchronized (sGLThreadManager) {
                    while (true) {
                        ...
                        if (mPaused != mRequestPaused) {
                            ...
                            if (LOG_PAUSE_RESUME) {
                                Log.i("gltext", "mPaused is now " + mPaused + " tid=" + getId());
                            }
                        }
                        ...
                        if (pausing && mHaveEglSurface) {
                            if (LOG_SURFACE) {
                                Log.i("gltext", "releasing EGL surface because paused tid=" + getId());
                            }
                            stopEglSurfaceLocked();
                        }
                        ...
                        if (pausing && mHaveEglContext) {
                            ...
                            if (!preserveEglContextOnPause) {
                                stopEglContextLocked();
                                if (LOG_SURFACE) {
                                    Log.i("gltext", "releasing EGL context because paused tid=" + getId());
                                }
                            }
                        }
                        ...
                        if ((!mHasSurface) && (!mWaitingForSurface)) {
                            if (LOG_SURFACE) {
                                Log.i("gltext", "noticed surfaceView surface lost tid=" + getId());
                            }
                            if (mHaveEglSurface) {
                                stopEglSurfaceLocked();
                            }
                        ...
                        if (mHasSurface && mWaitingForSurface) {
                            if (LOG_SURFACE) {
                                Log.i("gltext", "noticed surfaceView surface acquired tid=" + getId());
                            }
                        ...
                        if (doRenderNotification) {
                            if (LOG_SURFACE) {
                                Log.i("gltext", "sending render notification tid=" + getId());
                            }
                            mWantRenderNotification = false;
                        ...
                        if (LOG_THREADS) {
                            Log.i("gltext", "waiting tid=" + getId()
                ...
                if (createEglSurface) {
                    if (LOG_SURFACE) {
                        Log.w("gltext", "egl createSurface");
                    }
                ...
                if (sizeChanged) {
                    if (LOG_RENDERER) {
                        Log.w("gltext", "onSurfaceChanged(" + w + ", " + h + ")");
                    }
                ...
                if (mChoreographerRenderWrapper.canSwap()) {
                    if (LOG_RENDERER_DRAW_FRAME) {
                        Log.w("gltext", "onDrawFrame tid=" + getId());
                    }
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            if (LOG_SURFACE) {
                                Log.i("gltext", "egl context lost tid=" + getId());
                            }
                            lostEglContext = true;
                            break;
                        default:
    ...
    public void surfaceCreated() {
        synchronized (sGLThreadManager) {
            if (LOG_THREADS) {
                Log.i("gltext", "surfaceCreated tid=" + getId());
            }
            mHasSurface = true;
    ...
    public void surfaceDestroyed() {
        synchronized (sGLThreadManager) {
            if (LOG_THREADS) {
                Log.i("gltext", "surfaceDestroyed tid=" + getId());
            }
            mHasSurface = false;
            sGLThreadManager.notifyAll();
    ...
    public void onPause() {
        synchronized (sGLThreadManager) {
            if (LOG_PAUSE_RESUME) {
                Log.i("gltext", "onPause tid=" + getId());
            }
            mRequestPaused = true;
            sGLThreadManager.notifyAll();
            while ((!mExited) && (!mPaused)) {
                if (LOG_PAUSE_RESUME) {
                    Log.i("gltext", "onPause waiting for mPaused.");
                }
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
    ...
    public void onResume() {
        synchronized (sGLThreadManager) {
            if (LOG_PAUSE_RESUME) {
                Log.i("gltext", "onResume tid=" + getId());
            }
            ...
            while ((!mExited) && mPaused && (!mRenderComplete)) {
                if (LOG_PAUSE_RESUME) {
                    Log.i("gltext", "onResume waiting for !mPaused.");
                }
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
    ...
    public void onWindowResize(int w, int h) {
        synchronized (sGLThreadManager) {
            ...
            while (!mExited && !mPaused && !mRenderComplete
                    && ableToDraw()) {
                if (LOG_SURFACE) {
                    Log.i("gltext", "onWindowResize waiting for render complete from tid=" + getId());
                }
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
    ...
    private static class GLThreadManager {
        ...
        public synchronized void threadExiting(GLThread thread) {
            if (LOG_THREADS) {
                Log.i("gltext", "exiting tid=" + thread.getId());
            }
            thread.mExited = true;
            if (mEglOwner == thread) {
                mEglOwner = null;
    ...
    private static abstract class BaseConfigChooser
        ...
        public android.opengl.EGLConfig chooseConfig(android.opengl.EGLDisplay display, boolean recordable) {
            ...
            if (!EGL14.eglChooseConfig(display, attribList, 0, configs, 0, configs.length,
                    numConfigs, 0)) {
                Log.w("gltext", "unable to find RGB8888 / " + contextClientVersion + " EGLConfig");
                return null;
            }
            return configs[0];
        }
    ...
    public static class DefaultContextFactory implements EGLContextFactory {
        ...
        public void destroyContext(EGL10 egl, EGLDisplay display,
            ...
            if (!egl.eglDestroyContext(display, context)) {
                Log.e("gltext", "DefaultContextFactory display:" + display + " context: " + context);
                if (LOG_THREADS) {
                    Log.i("gltext", "DefaultContextFactory tid=" + Thread.currentThread().getId());
                }
    ...
    public static class DefaultWindowSurfaceFactory implements EGLWindowSurfaceFactory {
        ...
        public EGLSurface createWindowSurface(EGL10 egl, EGLDisplay display,
            ...
            try {
                ...
                Log.e("gltext", "eglCreateWindowSurface", e);
            }
            return result;
        }
